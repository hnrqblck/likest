...
export function InShareModal(props) {
    ...
    React.useEffect(() => {
        axios.get('https://master--linkest-server.netlify.app/.netlify/functions/app/home').then(resp => setLinkedinLink(resp.data));
        if (search) {
            const code = new URLSearchParams(search).get('code');
            const state = new URLSearchParams(search).get('state');
            postToken(code, state);
            setTimeout(() => {
                axios.get('https://master--linkest-server.netlify.app/.netlify/functions/app/token').then(resp => setPost(resp.data))
            }, 5000);
    ...
    async function postToken(code, state) {           
        try {
          await axios.post('https://master--linkest-server.netlify.app/.netlify/functions/app/token',
           {
            code,
            state
          })
    ...
    async function postImage(img, certType) {
        ...
        try {
          await axios.post('https://master--linkest-server.netlify.app/.netlify/functions/app/image',
           {
            img,
            certType,
            level
